      ...
      <div className="relative aspect-[16/10] md:aspect-[21/9] rounded-[2rem] overflow-hidden mb-4 shadow-2xl border border-zinc-200 dark:border-white/5 bg-zinc-200 dark:bg-zinc-900 reveal-item group">
        ...
        <div className="absolute bottom-6 right-6 left-6 text-right">
          <div className="flex flex-col gap-1">
            <span className="text-yellow-500 font-black text-[10px] tracking-[0.2em] uppercase">فئة القائمة</span>
            ...
            return (
              <div 
                ...
                className={`py-2 group transition-all -mx-2 px-2 rounded-2xl border border-transparent flex gap-2
                  ...
                  ${isAdmin ? '' : 'hover:bg-yellow-50/30 dark:hover:bg-yellow-900/5 hover:border-yellow-100 dark:hover:border-yellow-900/10'}`}
              >
                {isAdmin && (
                  ...
                  <div className="flex flex-wrap items-center justify-end gap-2">
                    <div className="flex items-center gap-1 order-1">
                      {isAdmin ? (
                        ...
                        <>
                          {item.isPopular && <span className="bg-yellow-500 text-black text-[8px] font-black px-1.5 py-0.5 rounded-md animate-popular">مميز</span>}
                          {item.isSpicy && <span className="animate-spicy text-xs leading-none">🌶️</span>}
                        </>
                      )}
                    </div>
                    <span className="text-zinc-900 dark:text-zinc-100 font-black text-lg leading-tight group-hover:text-yellow-600 transition-colors order-2 text-right w-full">
                      {item.name}
                    </span>
                  </div>
                  {item.description && (
                  ...
                  {item.prices.map((price, pIdx) => (
                    ...
                    >
                      ...
                      <div className={`${hasManyPrices ? '' : 'px-2 py-1 min-w-[45px] rounded-xl border transition-all flex items-center justify-center gap-1 shadow-sm'}
                        ${!hasManyPrices && isAdmin ? 'bg-yellow-600/10 border-yellow-600/30' : ''} 
                        ${!hasManyPrices && !isAdmin ? 'bg-zinc-100 dark:bg-zinc-800/80 group-hover:bg-white dark:group-hover:bg-zinc-800 border-zinc-200 dark:border-white/5 group-hover:border-yellow-500/40' : ''}
                      `}>
                          {isAdmin && onUpdatePrice ? (
                            <input 
                              ...
                              className="w-10 bg-transparent text-center font-black text-yellow-600 outline-none text-xs"
                            />
                          ) : (
                            <>
                              {isNumeric(price) ? (
                                <div className="flex items-center justify-center gap-0.5">
                                  <span className={`${hasManyPrices ? 'text-yellow-600 dark:text-yellow-500 text-lg' : 'text-yellow-600 text-base'} font-black leading-none tabular-nums`}>{price}</span>
                                  <span className={`${hasManyPrices ? 'text-yellow-600/60 dark:text-yellow-500/60 text-[9px]' : 'text-zinc-500 text-[8px]'} font-black`}>ج</span>
                                </div>
                              ) : (
                                <span className="text-yellow-600 font-black text-[9px] text-center leading-tight">{price}</span>
                              )}
                            </>
                          )}
                      </div>
